...
2. **Traumatic** — produced by trauma events when the trauma spec (`cdu0`) lands. The canonical case: ally killed by fire in same area → tribute acquires `Phobia(Fire)` with `Origin: Traumatic`. Trauma-induced phobias spawn at `Mild` and can escalate via the cascade rules from spec #1.
...
| `Traumatic` | **Habituation**: 5 cycles without the trigger firing → -1 tier. Tier reaches `Mild` then cures. Counter resets every time the trigger fires. |
| Either | **Sedative item**: suppresses the next firing of one specified phobia (does not remove the phobia, just one cycle of reaction). |
| Either | **Therapy sponsor gift**: removes one phobia entirely. (Filed for `dvd`.) |

Habituation is a tribute-side counter (`cycles_since_last_fire: u32`) on the phobia metadata, ticked each cycle in the cascade pass from spec #1. Reset to 0 whenever `is_present` returns true for this trigger that cycle.
...
`kind()` and `involves()` exhaustive matches gain five new arms (per the maintenance burden documented in `i26a`).
...
- Habituation: counter resets on fire; ticks down severity at threshold.
...
- **Habituation reset on fire**: any cycle where `is_present` returns true → `cycles_since_last_fire` becomes 0.
...
- **PR3**: Observer state (`observed_by`, `observer_seen_cycle`). Visibility-on-firing logic. Decay ticks. `knows_phobia` predicate consumed by alliance and brain decisions. Habituation counter for Traumatic origin. `PhobiaObserved`, `PhobiaHabituated`, `PhobiaForgotten` payloads.
...
- **Placeholders**: spawn-roll weights, freeze chance (25%), habituation cycles (5), observer decay cycles (5), trait-modifier table — explicit defaults to be tuned post-observability.
