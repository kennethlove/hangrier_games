...
pub(crate) async fn save_game(
    ...
    if !logs.is_empty() {
        ...
        let game_logs: Vec<super::GameLog> = logs
            .into_iter()
            .map(|log| super::GameLog {
        ...
        if current_count >= 9000 && current_count < super::MAX_MESSAGES as u32 {
            tracing::warn!(
        ...
        if total_after_insert > super::MAX_MESSAGES {
            let messages_to_delete = total_after_insert - super::MAX_MESSAGES;
            tracing::info!(
                game_id = %game.identifier,
                current_count = %current_count,
                new_count = %new_messages_count,
                deleting = %messages_to_delete,
                "Rotating old messages to maintain {} message limit", super::MAX_MESSAGES
            );
    ...
    let area_results = futures::future::join_all(game.areas.iter().map(|area| async {
        ...
        super::items::save_area_items(&area.items, id.clone(), db).await?;
    ...
    let tribute_results = futures::future::join_all(game.tributes.iter().map(|tribute| async {
        ...
        if tribute.is_alive() {
            super::items::save_tribute_items(&tribute.items, id.clone(), db).await?;
        }
