...
fn main() {
    ...
    let out_dir = PathBuf::from(&manifest_dir).join("src").join("templates");
    ...
    generate_sprite(&icons_dir.join("ui"), &out_dir.join("sprites_ui.svg"), "ui");
    generate_sprite(
        &icons_dir.join("narrative"),
        &out_dir.join("sprites_narrative.svg"),
        "narrative",
    );
